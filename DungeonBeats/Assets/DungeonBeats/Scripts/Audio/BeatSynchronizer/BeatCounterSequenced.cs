...
public class BeatCounterSequenced : MonoBehaviour {
    ...
    public int beatsCountInSequence = 0;
    [HideInInspector]
    ...
    IEnumerator BeatCheck()
    {
        while (audioSource.isPlaying)
        {
            ...
            {
                currentBeat++;
                beatInSequence = currentBeat % beatsCountInSequence;

                //Debug.Log(currentBeat * samplePeriod);
                //Debug.Log(audioSource.timeSamples-TempoUtils.instance.compensation);
                //Debug.Log("-------------");

                foreach (GameObject obj in observers)
                {
                    obj.GetComponent<BeatObserverSequenced>().BeatNotify(beatType, currentBeat, beatInSequence);
                }
